...
(function () {
 ...
 // === Day 11: Full Refresh 分支（counter 改由 full_refresh_scheduler.js 订阅 MESSAGE_RECEIVED 管）===
 // 原 Day 9 内联 counter 逻辑漏洞：applyInjection 仅在 bootstrap/手动 时调用，counter ++ 永远到不了 N。
 // 现在 scheduler 按 AI 回复轮次自然累加，满 N 时 applyInjection({force:true, fullRefresh:true}) 触达此处。
 if (opts.fullRefresh === true) {
   try {
     var yamlText = (typeof YAML !== 'undefined' && YAML && YAML.stringify)
       ? YAML.stringify(sd)
       : JSON.stringify(sd, null, 2);
     finalContent =
       '<!-- CFS Full Refresh: 全量 stat_data 快照（scheduler 周期触发） -->\n' +
       '```yaml\n' + yamlText + '\n```\n' +
       finalContent;
     L.info('🔄 Full Refresh 注入全量 stat_data (' + yamlText.length + ' 字符)');
   } catch (eFr) {
     L.warn('Full Refresh 序列化失败，本轮保持原 finalContent', eFr);
   }
 }
